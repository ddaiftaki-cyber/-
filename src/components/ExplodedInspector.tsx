...
export const ExplodedInspector: React.FC<ExplodedInspectorProps> = ({
  ...
  return (
    <section id="inspector" className="py-20 relative overflow-hidden bg-neutral-950/70 border-t border-b border-neutral-900 text-right">
      ...
      <div className="absolute top-1/2 right-0 w-96 h-96 bg-amber-900/10 rounded-full blur-[130px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-xs font-black text-amber-400 tracking-wider">
              <Layers className="w-4 h-4" />
              <span>المعاينة الهندسية وتفكيك طبقات الجودة 3D</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-black text-neutral-100 tracking-tight">
              سر الجودة في مفروشات ديموس: ما تحت القماش
            </h2>
            <p className="text-neutral-300 max-w-xl text-sm sm:text-base leading-relaxed">
              انقر على أي طبقة لمعاينة تفاصيل الخشب الزان، النوابض الجيبية المعزولة، والميموري فوم الطبي الذي يمنحك راحة تدوم 10 سنوات بدون أي هبوط.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all border shadow-lg ${
                config.isExploded
                  ? 'bg-amber-400 text-neutral-950 border-amber-300 shadow-amber-500/25'
                  : 'bg-neutral-900 border-neutral-800 text-neutral-200 hover:border-amber-500/40 hover:text-white'
              }`}
            >
              <Box className="w-4 h-4" />
              <span>{config.isExploded ? 'إعادة تجميع الكنب' : 'تفكيك طبقات الكنب 3D (Exploded)'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-3">
            <span className="text-xs font-bold text-neutral-400 px-1 block">
              اختر الطبقة لمعاينتها في المجسم:
            </span>
                ...
                return (
                  <div
                    ...
                    className={`group p-4 rounded-2xl border transition-all duration-200 cursor-pointer flex items-center justify-between ${
                      isSelected
                        ? 'bg-neutral-900/90 border-amber-400 shadow-lg shadow-amber-950/50 scale-[1.01]'
                    ...
                    <div className="flex items-center gap-3.5">
                      <div
                        className={`p-2.5 rounded-xl transition-colors ${
                          isSelected
                            ? 'bg-amber-400/20 text-amber-300 ring-2 ring-amber-500/30'
                      ...
                      <div className="space-y-0.5">
                        <div
                          className={`font-bold text-sm transition-colors ${
                            isSelected ? 'text-amber-400' : 'text-neutral-200 group-hover:text-neutral-100'
                          }`}
                        >
                          {hotspot.title}
                        </div>
          ...
          <div className="lg:col-span-7 p-6 sm:p-8 rounded-3xl bg-neutral-900/60 border border-neutral-800 backdrop-blur-xl relative overflow-hidden shadow-2xl">
            {/* Ambient inner glow */}
            ...
            <div className="space-y-6 relative z-10">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs font-mono text-amber-400">
                  <span className="p-1.5 rounded-lg bg-amber-950/80 border border-amber-800/60 font-bold">
                ...
                <h3 className="text-2xl sm:text-3xl font-black text-neutral-100 tracking-tight">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {activeAnnotation.specs.map((spec, idx) => (
                    <div
                      key={idx}
                      className="p-3.5 rounded-xl bg-neutral-950/80 border border-neutral-800 flex flex-col justify-between"
                    >
                      <span className="text-xs text-neutral-400">{spec.label}</span>
                      <span className="font-black text-sm text-amber-300 pt-1 font-sans">{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-neutral-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-neutral-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>مشمول بالضمان الذهبي الشامل 10 سنوات</span>
                </div>
                ...
                <button
                  ...
                  className="text-amber-400 hover:text-amber-300 font-black flex items-center gap-1"
                >
                  <span>تدوير ومعاينة الكنب 360°</span>
